e Evaluation Logic Stress-Test: Alternative Explanations and Falsification
...
## Evaluation Logic and Falsification

This behavior is evaluated using outcome-based indicators defined by task structure, rather than inferred internal state or linguistic markers. The evaluation logic is explicitly stress-tested against alternative explanations that reviewers might propose, using equivalence testing and confidence-interval–based reasoning (no fixed thresholds).

A full stress-test of the evaluation logic, including statistical framing and reviewer-facing falsification criteria, is provided in
analysis/eval_logic/cross_turn_retention_falsification.md.

Below we summarize the core alternative explanations considered and the conditions under which the behavior claim would be invalidated.

⸻

Alternative Explanations Considered
1. Instruction-Following Confound
Models may succeed by following implicit task instructions rather than retaining information across turns.

Ruling logic:
If instruction-following prompts increase apparent retention signals (e.g., correct parameter usage) without improving task success, or if instruction × retention interaction effects dominate the retention main effect, then success is attributable to instruction compliance rather than retention.

⸻

2. Task Ease Confound
The task may be solvable via simple parameter passing or adjacency-based strategies rather than cross-turn retention.

Ruling logic:
If trajectories that violate retention dependencies (e.g., re-querying prior information or omitting required parameters) succeed at rates statistically equivalent to trajectories with correct retention, then retention is not necessary for success.

⸻

3. Verbosity Confound
Retention signals may correlate with verbosity or general expressiveness rather than functional information retention.

Ruling logic:
If retention correlates more strongly with verbosity than with task success after controlling for verbosity, or if verbosity-matched zero-retention trajectories succeed equivalently, retention is a stylistic byproduct rather than a functional behavior.

⸻

4. General Capability Confound
Retention may be a proxy for overall model capability rather than a distinct behavior.

Ruling logic:
If retention correlates more strongly with general capability metrics (controlling for task success) than with task success itself, or if capability-matched trajectories with broken retention succeed equivalently, retention does not constitute a distinct behavioral dimension.

⸻

Falsification Conditions
The cross-turn information retention behavior is considered invalidated if any of the following are observed:
	•	Trajectories that violate retention dependencies succeed at rates statistically equivalent to those that satisfy them.
	•	Retention signals correlate more strongly with verbosity or general capability than with task success after appropriate controls.
	•	Instruction-following manipulations increase retention signals without improving task outcomes.
	•	Structural retention signals fail to predict success when implicit retention (as defined by task-grounded necessity) is absent.

These criteria ensure that retention is treated as a functionally necessary, behaviorally distinct capability, not a correlate of verbosity, instruction compliance, or general model strength.
